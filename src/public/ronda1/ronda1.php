<?php
    ...
    $sql = "SELECT nombre from preguntas_primerEscalon order by rand() limit 1";
...
<div>
    ...
    $sql = "SELECT respuesta from respuestas_primerEscalon where pregunta = '$pregunta'  order by rand()" ;
    $result = $conn->query($sql);
   ...
   <a  style="text-decoration:none" class= "display-6 text-white" align="center" href="../../public">Retirarse</a> 
</button> <br> <br>
...
<!-- Respuesta correcta -->
<?php
    $sql = "SELECT respuesta from preguntas_primerEscalon where nombre LIKE '$pregunta'";
    $result = $conn->query($sql);
    while ($row = $result->fetch_assoc()) {
